...
class Kuka:
  ...
  def applyAction(self, motorCommands):
    ...
    if (self.useInverseKinematics):
      ...
      if (self.useNullSpace==1):
        ...
        if (self.useOrientation==1):
          jointPoses = p.calculateInverseKinematics(self.kukaUid,self.kukaEndEffectorIndex,pos,orn,maxNumIterations=100)#,jointDamping=self.jd)
        else:
  ...
  def moveKukaEndtoPos(self, newxy, orn=None):
      ...
      jointPositions=[ 0.006418, 0.413184, -0.011401, -1.589317, 0.005379, 1.137684, -0.006539, 0.000048, -0.299912, 0.000000, -0.000043, 0.299960, 0.000000, -0.000200 ]
      for i in range (self.kukaEndEffectorIndex+1):
        #print(i)
        p.setJointMotorControl2(bodyUniqueId=self.kukaUid,jointIndex=i,controlMode=p.POSITION_CONTROL,targetPosition=jointPositions[i],targetVelocity=0,force=self.maxForce,maxVelocity=self.maxVelocity, positionGain=0.3,velocityGain=1)
      for sec in range(50):
        p.stepSimulation()
      for kuka_sec in range(1000):
          ...
          p.stepSimulation()
          time.sleep(0.001)
      error = np.array(p.getLinkState(self.kukaUid,self.kukaEndEffectorIndex)[0])-np.array(newxy)
      if np.abs(error[0]) > 0.001 or np.abs(error[1]) > 0.001 or np.abs(error[2]) > 0.201:
        print('resetting not correct')
        return False
      else:
        return True
